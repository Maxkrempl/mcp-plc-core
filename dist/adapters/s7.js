...
export class S7Adapter {
    ...
    async readAll() {
        ...
        return new Promise((resolve, reject) => {
            this.conn.readAllItems((bad, values) => {
                if (bad)
    ...
    async writeSetpoint(parameter, value) {
        ...
        return new Promise((resolve, reject) => {
            this.conn.writeItems(tag, value, (bad) => {
                if (bad)
                    reject(new Error(`S7 write error for ${parameter}`));
                else
                    resolve(true);
            });
        });
