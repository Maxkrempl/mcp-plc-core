...
export function createMCPServer(opts) {
    ...
    // TOOLS
    // ──────────────────────────────────────────────────────────
    server.tool("read_sensor", "Read a sensor value from the PLC.", {
        ...
        try {
            ...
            if (value === undefined) {
                ...
                return {
                    content: [{ type: "text", text: `Unknown sensor: "${sensor}". Available: ${available}` }],
                    isError: true,
                };
            }
            return {
            ...
            return {
                content: [{ type: "text", text: `Error reading sensor: ${err}` }],
                isError: true,
            };
        }
    });
    server.tool("get_alarms", "Get all active alarms from the PLC.", {}, async () => {
        try {
            ...
            return {
                content: [{ type: "text", text: `Error reading alarms: ${err}` }],
                isError: true,
            };
        }
    });
    server.tool("get_status", "Get overall PLC system status.", {}, async () => {
        try {
            ...
            return {
                content: [{ type: "text", text: `Error reading status: ${err}` }],
                isError: true,
            };
        }
    });
    server.tool("get_setpoints", "Get current setpoint values.", {}, async () => {
        try {
            ...
            return {
                content: [{ type: "text", text: `Error reading setpoints: ${err}` }],
                isError: true,
            };
        }
    });
    server.tool("write_setpoint", "Write a setpoint value to the PLC. Validates against safety limits.", {
        ...
        if (!adapter.writeSetpoint) {
            return {
                content: [{ type: "text", text: `Write not supported by ${adapter.name} adapter.` }],
                isError: true,
            };
        }
        // Check safety limits
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: `Error writing setpoint: ${err}` }],
                isError: true,
            };
        }
    });
    server.tool("get_all", "Get all PLC data at once: sensors, alarms, status, setpoints, outputs.", {}, async () => {
        try {
    ...
    // RESOURCES
    // ──────────────────────────────────────────────────────────
    server.resource("tags", "plc://tags", async () => {
        const sensors = await adapter.readSensors();
        return {
            contents: [{
                    uri: "plc://tags",
    ...
    server.resource("alarms", "plc://alarms", async () => {
        const alarms = await adapter.readAlarms();
        return {
            contents: [{
                    uri: "plc://alarms",
    ...
    server.resource("status", "plc://status", async () => {
    ...
    server.resource("info", "plc://info", async () => ({
        contents: [{
                uri: "plc://info",
                mimeType: "application/json",
                text: JSON.stringify({
